...
import { checkDuplicateProfile } from "./middlewares/verifyProfile";
import { createUser } from "./services/userService";
import { createProfile } from "./services/profileService"; // hw2: created profile
...
export default function setupRoutes(app) {
	...
	// hw2: add post for profile
	router.post("/profiles", checkDuplicateProfile, createProfile);
...
async function getStaticFile(res, filePath) {
	return fs
		.readFile(path.resolve(filePathPrefix, filePath), "utf8")
		.catch((err) => {
